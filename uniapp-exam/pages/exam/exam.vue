<template>
  <view>
    ...
    <!-- <view class="cu-modal padding " :class="modalError=='modalError'?'show':''" @tap="hideErrorModal">
			<view class="cu-dialog bg-white" @tap.stop>								
				...
				<radio-group class="block" >
					...
					<view class="cu-item cu-item-error" v-for="error in errorList" :key="error">
    ...
    <form>
      <swiper :current="subjectIndex" class="swiper-box" @change="SwiperChange" :style="{ height: swiperHeight }">
        <swiper-item v-for="(subject, index) in subjectList" :key="index">
          <scroll-view scroll-y="true" show-scrollbar="true" :style="{ height: swiperHeight }">
            <view v-if="index - subjectIndex >= -1 && index - subjectIndex <= 1">
              ...
              <view>
                ...
                <view v-else-if="subject.type === 5">
                  ...
                  <view class="cu-form-group solid margin"><textarea maxlength="-1" @blur="textInput" v-model="subject.userAnswer" placeholder="多行文本输入框"></textarea></view>
                </view>
              </view>
    ...
    <view id="foot-box" class="cu-bar tabbar bg-white shadow foot">
      ...
      <view v-if="enableShowAnswer" class="action" @click="ShowAnswerChange">
        <view class="cuIcon-cu-image"><text class="lg text-gray cuIcon-question"></text></view>
        <view class="text-gray">解答</view>
      </view>

      <view v-if="!enableShowAnswer" class="action" @click="submitAnswer">
        <view class="cuIcon-cu-image"><text class="lg text-gray cuIcon-question"></text></view>
        <view class="text-gray">交卷</view>
      </view>

      <view class="action" @click="backToIndex">
        <view class="cuIcon-cu-image"><text class="lg text-gray cuIcon-question"></text></view>
...
export default {
  ...
  onLoad(op) {
    ...
    this.enableShowAnswer = op.currentQs == -1;
    this.getExam(op)
      .then(res => {
  ...
  methods: {
    ...
    getExam(op) {
      return request('get-exam', op)
        ...
        .then(res => {
          if (res.errcode) {
            uni.showModal({
              content: res.errmsg,
              showCancel: false,
              success: () => uni.switchTab({ url: '../index/index' })
            });
        ...
        .catch(log('get-exam error: '));
    },
    submitAnswer() {
      //console.log('submitAnswer: ', this.subjectList);
      // const answers = this.subjectList.map(s => {
      //   const { question_id, answer, userAnswer } = s;
      //   return { question_id, answer, userAnswer };
      // });

      const answers = this.subjectList.reduce((last, next) => ({ userAnswer: last.userAnswer + ',' + next.userAnswer }));
      console.log('submit answers: ', answers);
      request('answer', { exam_name: this.title, answers: answers.userAnswer })
        .then(log('answer return: '))
        .then(res => {
          if (res) {
            this.enableShowAnswer = true;
            uni.showModal({ content: `交卷成功，你的考试成绩为${res.content.score}分`, showCancel: false, success: () => {} });
          }
          res.errcode && uni.showModal({ content: res.errmsg, showCancel: false, success: () => {} });
        })
        .catch(log('answer error: '));
    },
